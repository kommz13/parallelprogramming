...
inline void calculateNextState(int columns, int rows, char *u1, char *u2, int x, int y);
...
int life(const int rank, const int N, MPI_Comm *cartComm, const int GRID_SIDE, const int REDUCE)
{
    ...
    { 
        end_time = MPI_Wtime();
        printf("Elapsed time = %f seconds\n", end_time - start_time); //print total time upologismwn
    }
...
void calculateNextState(int columns, int rows, char *u1, char *u2, int x, int y)
{
...
{ //update internal grid of process block
...
#pragma omp for reduction(+ \
    ...
    {
        calculateNextState(columns, rows, u1, u2, x, 1);
    }
...
#pragma omp for reduction(+ \
    ...
    {
        calculateNextState(columns, rows, u1, u2, 1, y);
    }
...
#pragma omp for reduction(+ \
    ...
    for (int y = 1; y <= rows - 2; y++)
    {
        calculateNextState(columns, rows, u1, u2, columns - 2, y);
    }
